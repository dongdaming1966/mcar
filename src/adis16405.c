...
int adis16405_init(void)
{
	...
	if((receive[0]==0x40)&&(receive[1]==0x15))
		printf("[ADIS] product id checked.\n");
	else
	{
		printf("[ADIS] error: product id is wrong received %x%x\n",receive[0],receive[1]);
		return -1;
	}
	...
	if((receive[0]==0x00)&&(receive[1]==0x06))
		printf("[ADIS] MSC_CTL checked.\n");
	else
	{
		printf("[ADIS] error: MSC_CTL is wrong received %x%x\n",receive[0],receive[1]);
		return -1;
	}
	...
	if((receive[0]==0x00)&&(receive[1]==0x01))
		printf("[ADIS] SIMPL_PRD checked.\n");
	else
	{
		printf("[ADIS] error: SIMPL_PRD is wrong received %x%x\n",receive[0],receive[1]);
		return -1;
	}
	...
	if((receive[0]==0x04)&&(receive[1]==0x02))
		printf("[ADIS] SENS_AVG checked.\n");
	else
	{
		printf("[ADIS] error: SENS_AVG is wrong received %x%x\n",receive[0],receive[1]);
		return -1;
	}
	return fd; 
